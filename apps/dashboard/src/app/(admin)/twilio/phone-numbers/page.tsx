...
export default function PhoneNumbersPage() {
  ...
  const fetchPhoneNumbers = async () => {
    try {
      ...
      const [res, agentsRes] = await Promise.all([
        ...
        fetch(`${API_URL}/vapi/admin/user-agents`, {
          headers: { Authorization: `Bearer ${token}` }
        })
      ]);
  ...
  const handleUnassignDirect = async (numberId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.details || data.error || 'Failed to unassign user');
      }
  ...
  const handleAssignUser = async (userId: string | null) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.details || data.error || 'Failed to assign user');
      }
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200 text-sm text-gray-500">
                ...
                <th className="px-6 py-4 font-medium">Active Agent</th>
            ...
            <tbody className="divide-y divide-gray-100 text-sm">
              {phoneNumbers.length === 0 ? (
                ...
                phoneNumbers.map((number: any) => {
                  ...
                  const availableAgents = userAgents.filter(a => a.userId === number.userId);
                  return (
                    <tr key={number.id} className="hover:bg-gray-50/50 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-gray-400 mt-1">ID: {number.sid?.substring(0, 8) || ''}...</div>
                      </td>
                      <td className="px-6 py-4 text-gray-600">{number.locality || '-'}</td>
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
                          {number.capabilities?.voice && (
                            <span title="Voice"><Phone className="w-4 h-4 text-green-500" /></span>
                          )}
                          {number.capabilities?.sms && (
                            <span title="SMS"><MessageSquare className="w-4 h-4 text-blue-500" /></span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        {number.user ? (
                          <div className="flex items-center gap-2 bg-blue-50 text-blue-700 px-3 py-1.5 rounded-full w-fit border border-blue-100">
                      ...
                      <td className="px-6 py-4">
                        {number.user ? (
                          <select
                            className="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 max-w-[150px]"
                            value={number.activeAgentId || ''}
                            onChange={(e) => handleAssignAgent(number.id, e.target.value)}
                          >
                            <option value="">No Agent (Forward)</option>
                          ...
                          <span className="text-xs text-gray-400 italic">Assign user first</span>
                        )}
                      </td>
